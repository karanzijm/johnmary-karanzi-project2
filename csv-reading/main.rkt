...
;;
;; @defproc %csv-reading:csv-spec-derive orig-spec changes
;;
...
;;
;; @lisp
;; (%csv-reading:csv-spec-derive my-first-csv-spec
;;                  '((separator-chars . (#\%))
;;                    (comment-chars   . (#\#))))
;; @result{}
;; ((separator-chars         . (#\%))
...
(define (%csv-reading:csv-spec-derive orig-spec changes)
...
(define (%csv-reading:csv-error code extra)
...
(define (%csv-reading:newline-check-step0 newline-type c port)
  ...
  (case newline-type
    ...
    ((lax detect) (cond ((eqv? c #\return)
                        ...
                        ((eqv? c #\newline) 'lf)
                        (else #f)))
    (else (error '%csv-reading:make-portreader/positional
                 "unrecognized newline-type: ~S"
                 newline-type))))

(define %csv-reading:make-portreader/positional
  (letrec-syntax
      ((newline-check
        (syntax-rules ()
          ((_ newline-type c port detected-newline-type)
           ...
           (if (eqv? newline-type 'detect)
               (begin (set! detected-newline-type
                            (%csv-reading:newline-check-step0 newline-type c port))
...
(define %csv-reading:make-portreader
    ...
    (lambda (reader-spec)
      (let ((newline-type               'lax)
        ...
        (for-each
         ...
         reader-spec)
        (%csv-reading:make-portreader/positional
         newline-type
         separator-chars
         quote-char
         quote-doubling-escapes?
...
(define (make-csv-reader-maker reader-spec)
  (let ((make-portread
         (if (let ((p (assq 'newline-type reader-spec))) (and p (cdr p)))
             ...
             (letrec
                 ((detect-portread
                   (%csv-reading:make-portreader
                    (%csv-reading:csv-spec-derive reader-spec
                                                  '((newline-type . detect)))))
             ...
             (let ((reusable-portread
                    (%csv-reading:make-portreader reader-spec)))
               (lambda () reusable-portread)))))
    (lambda (in)
      (let ((port     (%csv-reading:in-arg '<csv-reader> in))
    ...
    (lambda (in . rest)
      (let ((spec (cond ((null? rest)       '())
        ...
        ((if (null? spec)
         ...
         (%csv-reading:in-arg 'make-csv-reader in))))))
...
(define (csv-for-each proc reader-or-in)
  (let ((reader (%csv-reading:reader-or-in-arg 'csv-for-each reader-or-in)))
    (let loop ((row (reader)))
      (if (null? row)
          (void)
          (begin (proc row)
...
(define (csv-map proc reader-or-in)
  (let ((reader (%csv-reading:reader-or-in-arg 'csv-map reader-or-in)))
...
;; (define (csv->list reader-or-in)
;;   (let ((reader (%csv-reading:reader-or-in-arg 'csv->list reader-or-in)))
;;     (let ((head '()))
;;       (let ((row (reader)))
;;         (if (null? row)
;;             head
...
(doc (section "Converting CSV to SXML")
...
(define csv->sxml
  (let* ((top-symbol
    ...
    (lambda (reader-or-in . rest)
      (let ((reader       (%csv-reading:reader-or-in-arg 'csv->sxml
                                                         reader-or-in))
            (row-element  'row)
            (col-elements #f))
...
;; TODO: 2016-02-21 Maybe find better ways to distribute these embedded tests
;; throughout the file, now that we've moved them into the file from
;; test-csv.rkt (during the move from PLaneT to new package system).
...
(module+ test
  ...
  (test-section 'whitespace-stripping
  ...
  (test-section 'newline-type-adapt
...
(doc history
     ...
     (#:planet 3:5 #:date "2021-11-02"
               (itemlist
                (item (racket csv-for-each)
                      " now returns a void value.  (Thanks to Yury Bulka for
suggesting.)")))

     (#:planet 3:4 #:date "2016-12-09"
               (itemlist
                (item "Fixed bug of various "
                      (racket %csv-reading:reader-or-in-arg)
                      " calls passing string instead of symbol as first
argument, which resulted in an error when an error-raising call was
made. (Thanks to John B. Clements.)")))

     (#:planet 3:3 #:date "2016-03-02"
               (itemlist
                (item "Tweaked info.rkt, filenames.")
                (item "Linked to sxml-intro package, and changed ``SXML/xexp'' references to ``SXML''.")))

     (#:planet 3:2 #:date "2016-02-27"
               (itemlist
                (item "Updated home page URL.")
                (item "Separated deps and build-deps.")))

     (#:planet 3:1 #:date "2016-02-25"
               (itemlist
                (item "Fixed deps.")))

     (#:planet 3:0 #:date "2016-02-21"
               (itemlist
                (item "Moving PLaneT "
                      (code "csv")
                      " package to new package system as "
                      (code "csv-reading")
                      " package, due to naming conflict.")
                (item "Moved test suite into main source file.")))
     ...
     (#:version "0.11" #:date "2011-08-22" #:planet 1:7
                (itemlist
                 (item "Changed URL.")
                 (item "Changed references to Scheme to Racket.
A little code cleanup, including using Racket "
                       (racket error)
                       " better.")))
     ...
     (#:version "0.7" #:date "2009-02-22" #:planet 1:3
                (itemlist
                 (item "License is now LGPL 3.")
                 (item "Moved to author's new Scheme administration system.")))

     (#:version "0.6" #:date "2008-08-12" #:planet 1:2
                (itemlist
                 (item "For PLT 4 compatibility, new versions of "
                       (racket csv-map)
                       " and "
                       (racket csv->list)
                       " that don't use "
                       (racket set-cdr!)
                       ".  (Thanks to Doug Orleans.)")
                 (item "PLT 4 "
                       (racket if)
     ...
     (#:version "0.5" #:date "2005-12-09"
                (itemlist
                 (item "Changed a non-R5RS use of "
                       (racket letrec)
                       " to "
                       (racket let*)
     ...
     (#:version "0.1" #:date " 2004-05-31"
                (itemlist
                 (item "First release, for testing with real-world input."))))
